...
namespace APT_calibrator_tool
{
    ...
    public partial class FormAPTViewer : Form
    {
        ...
        private List<Point> APTseedPoints = new List<Point>();
        // 采集结果数组 [index, 0]=X, [index,1]=Y
        public double[,] APTSeedCoords { get; private set; }
        ...
        // 带已有 DynRec 实例的构造
        public FormAPTViewer(Form_DynRec existingDynRec) : this()
        {
        ...
        {
            string input = ShowInputDialog("Collect Seeds", "Enter number of seeds (including a begining point):", "5");
            if (int.TryParse(input, out var n) && n > 0)
            {
                seedCount = n;
                APTseedPoints.Clear();
                APTSeedCoords = new double[seedCount, 2];
                textBoxSeeds.Clear();
        ...
        private void PictureBox1_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            switch (currentMode)
            {
                case Mode.CollectSeeds:
                    if (APTseedPoints.Count < seedCount)
                    {
                        APTseedPoints.Add(p);
                        int idx = APTseedPoints.Count - 1;
                        var origin = APTseedPoints[0];
                        ...
                        APTSeedCoords[idx, 0] = realX;
                        APTSeedCoords[idx, 1] = realY;
                        ...
                        if (APTseedPoints.Count == seedCount && dynForm != null && !dynForm.IsDisposed)
                        {
                            dynForm.APTSeedCoords = this.APTSeedCoords;
        ...
        private void PictureBox1_Paint(object sender, PaintEventArgs e)
        {
            var g = e.Graphics;
            // 绘制 seeds
            for (int i = 0; i < APTseedPoints.Count; i++)
            {
                var pt = APTseedPoints[i];
        ...
        {
            string input = ShowInputDialog("Scale Bar", "Enter real length (e.g. 50 nm):", "50 nm");
